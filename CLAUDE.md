...
| **[Native LLM SDK](./docs-internal/native_llm_sdk.md)** | Native SDK layer in `services/llm/`: protocol-based providers, config-driven base URLs, and 14 providers (12 cloud plus Ollama / LM Studio; xAI is agent-selectable without a standalone chat-model node). `bedrock` is the non-OpenAI-compat extension pattern — an `AnthropicProvider` subclass swapping in `AsyncAnthropicBedrock`, with two auth modes squeezed through the one credential slot the synchronous factory allows (a Bedrock API key, or the `aws-sigv4` sentinel meaning "sign with the host's own AWS credential chain"). A JSON-driven `supports_model_listing: false` flag covers endpoints that speak the OpenAI chat wire format but ship no `/v1/models` route (Sarvam; and Bedrock, whose inventory lives on a different control plane entirely) — `OpenAIProvider.fetch_models` then serves the curated list and probes the key with a one-token completion. Every provider is native; `tests/llm/test_langchain_removed.py` locks the retired-dependency end state. |
...
Chat-model nodes (`openaiChatModel`, `anthropicChatModel`, ...) render through `SquareNode` from the backend NodeSpec. Direct chat and every new agent execution route through the native SDK facade (`ChatUnifier` in `services/llm/`); the 14-provider surface is 12 cloud providers plus Ollama and LM Studio. Thirteen providers have standalone chat-model nodes under `server/nodes/model/`, while xAI is selected directly from agent parameters. `bedrock` reaches the same Claude models as `anthropic` through AWS — it is a subclass of `AnthropicProvider` with a different client, and its model ids are region-scoped inference profiles (`us.anthropic.*`), not bare model names. Model params (max output, context length, thinking type, temperature range) come from `ModelRegistryService`. The provider architecture, proxy/local-LLM routing, and the per-provider model + thinking/reasoning matrix (budget / effort / format) all live in **[Native LLM SDK](./docs-internal/native_llm_sdk.md)**. A Temporal history recorded before the native cutover carries no `llm_engine` marker and messages in a retired wire format; `agent.execute_llm_step` refuses it with a non-retryable `InvalidAgentLLMEngine` rather than misreading it, and the deployment must be Reset. The runtime output schema (`thinking` field for downstream nodes) is backend-served via `register_output_schema` (see [Plugin System](./docs-internal/plugin_system.md)).

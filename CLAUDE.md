...
├── package.json                 # Dependency: express (inlined by `bun build --target=bun`); engines.bun >= 1.4.0
...
```
server/nodejs/
├── package.json              # Dependency: express (inlined into the bundle). Scripts: build = `bun build src/index.ts --target=bun --outfile=dist/index.js`,

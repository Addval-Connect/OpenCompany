...
| `_install.py` | `ensure_vercel_cli()` — pinned `bun add vercel@<X>` (`core.js_runtime.add_package`) into the shared packages tree (`<DATA_DIR>/packages/`), run on bun (no Node or npm involved; vercel 54.21.1 verified on bun with no Node on PATH); system PATH preferred; one-shot `telemetry disable` post-install |
